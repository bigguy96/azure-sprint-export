...
public class AzureDevOpsService
{
    ...
    {
        return string.IsNullOrWhiteSpace(name) ? null : string.Join("-", name.Split(' ', StringSplitOptions.RemoveEmptyEntries)).Trim();
    }

    public async Task<List<Sprint>> GetSprintsAsync()
    {
        using var client = new HttpClient();
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(System.Text.Encoding.ASCII.GetBytes($":{_bearerToken}")));
